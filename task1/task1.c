...
main(int argc, char *argv[])
{
	...
	MPI_Comm_size(comm, &numOfSlaves);
	...
	case MASTER_RANK:
			...
			for (int s = 1;
			s < numOfSlaves && err == OK;
			s++)
		{
			InitTask(t, s);

			MPI_Send(b, sizeof(Task), MPI_CHAR, s, TASK_MSG, comm);
			MPI_Recv(b, sizeof(Task), MPI_CHAR, s, TASK_MSG, comm, &status);
	...
	default:
		...
		MPI_Recv(b, sizeof(Task), MPI_CHAR, MASTER_RANK, TASK_MSG, comm, &status);
		...
		MPI_Send(b, sizeof(Task), MPI_CHAR, MASTER_RANK, TASK_MSG, comm);
